defmodule Newbee.TUI.HistoryTest do
  ...
  setup do
    tmp = Path.join(System.tmp_dir!(), "newbee_hist_test_#{:erlang.unique_integer([:positive])}")
    File.mkdir_p!(tmp)
  ...
  test "append 后 load 能读回（跨“进程”持久，即模拟退出重进）", %{path: path} do
    History.append("帮我重构 foo")
    History.append("解释一下 diff")
    assert History.load() == ["帮我重构 foo", "解释一下 diff"]
  end

  test "append 去重：与最后一条相同则跳过", %{path: path} do
    History.append("a")
    History.append("a")
  ...
  test "append 空行不写", %{path: path} do
    History.append("   ")
    assert History.load() == []
  end

  test "顺序：最早的在前（与 Line.hist 一致，↑ 从最新翻起）", %{path: path} do
    History.append("one")
    History.append("two")
